			...
			function sendToHoriz() {
				...
				for(var f=0;f<vectorLayer.features.length;f++){
						...
						if(vectorLayer.selectedFeatures.indexOf(vectorLayer.features[f])> -1 ){
							...
							$.ajax({
						  type: "GET",
						  url: "http://" + serverIp + ":" + serverPort + "/ServerJAXRS/cmd/horizontalB/2/" + roomId + "/"+id 
						  })
						   .done(function( data ) {
							   console.log("command sent to server");
							   })
			...
			function sendToClosed() {
				...
				for(var f=0;f<vectorLayer.features.length;f++){
						...
						if(vectorLayer.selectedFeatures.indexOf(vectorLayer.features[f])> -1 ){
							...
							$.ajax({
						  type: "GET",
						  url: "http://" + serverIp + ":" + serverPort + "/ServerJAXRS/cmd/closedB/2/" + roomId + "/"+id 
						  })
						   .done(function( data ) {
							   console.log("command sent to server");
							   })
			...
			function sendToEnergy() {
				...
				for(var f=0;f<vectorLayer.features.length;f++){
						...
						if(vectorLayer.selectedFeatures.indexOf(vectorLayer.features[f])> -1 ){
							...
							$.ajax({
						  type: "GET",
						  url: "http://" + serverIp + ":" + serverPort + "/ServerJAXRS/cmd/energyB/2/" + roomId + "/"+id 
						  })
						   .done(function( data ) {
							   console.log("command sent to server");
							   })
